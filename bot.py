...
class Bot(object):
	...
	self.welcome_message = welcome_message
        ...
        self.newcomers = []
        self.hello_regex = re.compile(get_regex(hello_list, self.botnick), re.I)  # Regexed version of hello list
        self.help_regex = re.compile(get_regex(help_list, self.botnick), re.I)  # Regexed version of help list
...
def message_response(bot, ircmsg, actor, ircsock, channel, greeters):
    ...
    if bot.botnick.lower() and "PRIVMSG".lower() in ircmsg.lower():
        if bot.hello_regex.search(ircmsg):
            bot_hello(random.choice(hello_list), actor, ircsock, channel)
